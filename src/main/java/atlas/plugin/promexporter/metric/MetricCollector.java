...
public class MetricCollector {
    ...
    private static final List<String> metricsList = Arrays.asList(
            ...
            Parameter.BRANCH.getLabelName(), Parameter.JOB.getLabelName(),
    ...
    public static final Gauge JOB_QUEUE_DURATION = Gauge.build()
            .name("job_queue_duration").help("Job queue duration")
            .labelNames("ProjectName", "PlanName", "JobName").register();

    public static final Gauge JOB_PROCESSING_DURATION = Gauge.build()
            .name("job_processing_duration").help("Job processing duration")
            .labelNames("ProjectName", "PlanName", "JobName").register();

    public static final Gauge JOB_START_TIME = Gauge.build()
            .name("job_start_time").help("Job start time")
            .labelNames("ProjectName", "PlanName", "JobName").register();
    ...
    // XXX FAILED_TESTS
    public static final Counter FAILED_TESTS = Counter
            .build()
            .name(FAILED_TESTS_METRIC_NAME)
            ...
            .labelNames(Parameter.CLASS_NAME.getLabelName(),
                    ...
                    Parameter.PLAN.getLabelName(), "Error").register();

    public void removeByLabels(
            @SuppressWarnings("rawtypes") SimpleCollector collector,
            Map<Parameter, String> parameters) {

        List<MetricFamilySamples> metrics = Arrays.asList(collector.collect()
                .get(0));

        collector.clear();

        List<Sample> listSamples = metrics.get(0).samples.stream()
    ...
    public static void clearRegressStatistic() {
        JOB_PROCESSING_DURATION.clear();
        JOB_QUEUE_DURATION.clear();
        JOB_START_TIME.clear();
        JOB_END_TIME.clear();
        TEST_NUMBER.clear();
    }
